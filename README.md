...
「上次检查后」按首次收录时间查看新文章，并分别记住每个分组或期刊的检查起点。首次使用显示该范围已有文章；看完后点击「本次检查完成」，以当前数据批次的生成时间保存起点，不会顺便把文章标为已读。历史尚未加载完成、读取失败或仍有搜索/摘要筛选时，不能推进检查起点。这个入口不受“近 30/90 天”限制，较久未打开也不会因此漏掉期间收录的文章。
