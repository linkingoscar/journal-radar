...
v5 完整备份内嵌手动封面，导入前检查图片，已有手动选择优先。网页版也能显示、保留和再次导出导入的手动封面；上传和自动匹配仍由本机版提供。内置及自动匹配图片不重复打包，换机后可复用或重新匹配。旧 v4 备份没有图片，需另行保留 `.desktop-data/covers/`，参见[恢复说明](docs/data-recovery.md)。已有安装升级时执行 `.desktop-venv/Scripts/python.exe -m pip install -r radar/requirements.txt`，再重启本机组件。
...
「导出完整备份」生成 v5 JSON，包含已读、收藏、自选期刊、收藏文章分组、常用筛选、个人笔记与标签、相应文章与手动引用修正、已有完整译文、个人期刊组、新增期刊配置、检查起点、浏览偏好、自动翻译开关和手动封面，兼容导入 v1–v4。导出不请求翻译服务；导入先校验再合并，保留目标现有记录。新期刊随备份恢复，网页版可阅读已备份文章，本机版可继续采集；无需另找 library.json。邮箱、访问令牌、额度和全部历史目录不导出。单文件上限 50 MB；迁移与异常恢复见[数据恢复说明](docs/data-recovery.md)。
