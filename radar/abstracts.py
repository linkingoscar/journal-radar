...
def readable_abstract(value):
    ...
    if re.match(r"^.{0,150},\s*Volume\b", text, re.I) or re.search(
        r"not available|no abstract available", text, re.I
    ):
        return ""
    return text
...
def usable_abstract(value):
    """Reject obvious fragments without declaring every other abstract complete."""
    text = readable_abstract(value)
...
def valid_abstract(value):
    text = re.sub(
        r"^(?:Abstract|Summary)\s*[:：]?\s+", "", usable_abstract(value), flags=re.I
    )
    if (
        not 120 <= len(text) <= 20000
        or len(text.split()) < 20
        or text.endswith(("…", "..."))
        or re.search(
            r"enable javascript|verify you are human|access denied|checking your browser",
            text,
            re.I,
        )
    ):
...
def title_matches(expected, actual):
    ...
    return bool(a and b and SequenceMatcher(None, a, b).ratio() >= 0.9)
...
def record_matches(article, identifier, title):
    ...
    if article.get("journal_id") == "0823-9150" and doi(identifier).startswith(
        "10.1111/1911-3846."
    ):
        expected, actual = (
            re.sub(r"[^\w]", "", clean(x).casefold()) for x in (article["title"], title)
        )
        return (
            len(actual) >= 30
...
class ArticleHTML(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if (
            ...
            and re.search(
                r"(?:^|\s)(?:abstract|abstracts|abstractsection|abstractcontent|article-section__abstract|hlFld-Abstract)(?:\s|$)",
                identity,
                re.I,
            )
        ):
    ...
    def handle_data(self, data):
        ...
        elif self.capture and not any(
            t in ("script", "style", "nav", "button") for t in self.stack
        ):
...
def from_html(html, article):
    ...
    declared = [
        meta[k]
        for k in ("citation_doi", "dc.identifier", "dc.identifier.doi", "prism.doi")
        if meta.get(k) and "10." in meta[k]
    ]
    same = (
        any(doi(x) == doi(article.get("doi")) for x in declared)
        if declared
        else any(
            title_matches(article["title"], meta.get(k))
    ...
    for value in page.ld:
        for item in objects(value):
            if item.get("@type") in ("ScholarlyArticle", "Article") and title_matches(
                article["title"], item.get("headline") or item.get("name")
            ):
...
def allowed_url(url):
    try:
        ...
        return (
            ...
            and (
                host in API_HOSTS
                or any(host == h or host.endswith("." + h) for h in PUBLISHERS)
            )
        )
    except ValueError:
        return False
    ...
    for _ in range(6):
        ...
        with requests.get(
            ...
            if "text/html" in r.headers.get("Content-Type", ""):
                ...
                refresh = re.search(
                    r"url\s*=\s*[\'\"]?(.+?)[\'\"]?$",
                    page.meta.get("refresh", ""),
                    re.I,
                )
                if refresh:
                    url = urljoin(url, refresh[1])
                    continue
            return text, url
...
class AbstractService:
    def __init__(self, directory, getter=fetch):
        ...
        with self.connection() as c:
            c.execute(
                "CREATE TABLE IF NOT EXISTS radar_abstracts (article_id TEXT PRIMARY KEY, fingerprint TEXT, result TEXT)"
            )
            c.execute(
                "CREATE TABLE IF NOT EXISTS radar_source_backoff (host TEXT PRIMARY KEY, retry_at REAL, reason TEXT)"
            )
    ...
    def request(self, url):
        def before_request(target):
            with self.connection() as c:
                row = c.execute(
                    "SELECT retry_at,reason FROM radar_source_backoff WHERE host=?",
                    (source_host(target),),
                ).fetchone()
        ...
        try:
            ...
            if status == 429:
                try:
                    delay = max(
                        delay,
                        min(86400, float(response.headers.get("Retry-After", delay))),
                    )
                except ValueError:
                    pass
            if status != 404:
                target = (
                    (response.url if response is not None and response.url else None)
                    or getattr(getattr(exc, "request", None), "url", None)
                    or url
                )
                retry_at = time.time() + delay
                with self.connection() as c:
                    c.execute(
                        "INSERT OR REPLACE INTO radar_source_backoff VALUES (?,?,?)",
                        (source_host(target), retry_at, reason),
                    )
    ...
    def cached(self, article):
        with self.connection() as c:
            row = c.execute(
                "SELECT result FROM radar_abstracts WHERE article_id=? AND fingerprint=?",
                (article["id"], self.fingerprint(article)),
            ).fetchone()
    ...
    def save(self, article, result):
        ...
        with self.connection() as c:
            ...
            if old and usable_abstract(json.loads(old[0]).get("abstract")):
                return json.loads(old[0])
            c.execute(
                "INSERT OR REPLACE INTO radar_abstracts VALUES (?,?,?)",
                (
                    article["id"],
                    self.fingerprint(article),
    ...
    def overlay(self, payload):
        with self.connection() as c:
            cached = {
                ...
                for r in c.execute("SELECT * FROM radar_abstracts")
            }
        for article in payload["articles"]:
            ...
            if (
                item
                and item[0] == self.fingerprint(article)
                and item[1].get("resolved_doi")
            ):
                article["resolved_doi"] = item[1]["resolved_doi"]
            if (
                not usable_abstract(article["abstract"])
                and item
                and item[0] == self.fingerprint(article)
                and usable_abstract(item[1].get("abstract"))
            ):
                result = item[1]
                article.update(
    ...
    def resolve_doi(self, article):
        ...
        if (
            not re.fullmatch(r"\d{4}-\d{3}[\dX]", journal)
            or len(clean(article["title"])) < 30
        ):
            return None
        url = (
            ...
            + journal
            + "/works?"
            + urlencode({"query.bibliographic": article["title"], "rows": 5})
        )
        ...
        for record in json.loads(body).get("message", {}).get("items", []):
            ...
            if (
                record.get("type") == "journal-article"
                and re.fullmatch(r"10\.\d{4,9}/\S+", identifier)
                and set(record.get("ISSN", [])) & set(article.get("issns") or [journal])
                and re.sub(r"[^\w]", "", clean(title).casefold())
                == re.sub(r"[^\w]", "", clean(article["title"]).casefold())
            ):
    ...
    def due(self, article, full=False):
        ...
        if previous.get("abstract"):
            return not bool(usable_abstract(previous["abstract"]))
    ...
    def lookup(self, article, background=False):
        ...
        with self.lock:
            ...
            reasons, retries = [], []
            original = article
            identifier = doi(article.get("doi")) or (previous or {}).get(
                "resolved_doi", ""
            )
            ...
            def failure(provider, exc):
                reason = (
                    exc.reason
                    if isinstance(exc, SourceDeferred)
                    else "network_error"
                    if isinstance(exc, requests.RequestException)
                    else "unsupported_or_invalid"
                )
                errors.append(provider + ": " + reason)
                reasons.append(reason)
                if isinstance(exc, SourceDeferred):
                    retries.append(exc.retry_at)
            ...
            if identifier:
                for provider in ("OpenAlex", "Crossref"):
                    ...
                    if (
                        background
                        and provider == "OpenAlex"
                        and previous
                        and previous.get("version") == LOOKUP_VERSION
                        and previous.get("index_checked_at", 0) > time.time() - 86400
                    ):
                        continue
                    # A current Crossref feed record with an empty abstract need not be fetched again per article.
                    if (
                        background
                        and provider == "Crossref"
                        and "crossref" in (article.get("sources") or "").split(",")
                    ):
                        continue
                    try:
                        url = (
                        ...
                        if provider == "OpenAlex":
                            ...
                            text = (
                                valid_abstract(record.get("abstract"))
                                if record_matches(
                                    article,
                                    record.get("DOI"),
            ...
            messages = {
                "rate_limited": "来源限流，已暂停该来源，稍后补采会自动重试。",
        ...
        for offset in range(0, min(len(candidates), limit), 50):
            ...
            for article in batch:
                ...
                self.save(
                    article,
                    {
                        "status": "found" if text else "unavailable",
                        "attempt": "index",
                        "abstract": text,
                        "source": "OpenAlex",
                        "source_url": record.get("id", ""),
                ...
                found += bool(text)
            if progress:
                progress(checked, min(len(candidates), limit), found)
            time.sleep(1)
    ...
    def enrich(self, payload, stop, progress):
        ...
        if batch.get("error"):
            report["index_error"] = batch["error"]
        self.overlay(payload)
        pending = []
        for a in missing:
            if usable_abstract(a.get("abstract")):
                continue
            # Retain notices in the library; don't repeatedly scrape obvious non-research records.
            if re.match(
                r"^(?:issue information|editorial board|front matter|back matter|table of contents|erratum|corrigendum|correction to|retraction notice)(?:\b|:)",
                a["title"],
                re.I,
            ):
                report["reasons"]["notice"] = report["reasons"].get("notice", 0) + 1
                continue
            if self.due(a, full=True):
                pending.append(a)
            else:
                report["reasons"]["cached"] = report["reasons"].get("cached", 0) + 1
        ...
        for a in pending:
            ...
            reason = (
                ...
                else result.get("reason", "not_provided")
            )
...
if __name__ == "__main__":
    ...
    write_site(payload, path.parent)
