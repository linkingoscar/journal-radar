...
def merge_apa_aliases(articles):
    ...
    for article in articles:
        ...
        if not canonical or not all(
            article.get(k) for k in ("title", "authors", "volume", "pages")
        ):
            continue
        if not all(
            article.get(k) == canonical.get(k)
            for k in (
                "journal_id",
                "archive_year",
                "title",
...
def normalize_archive(record, journal):
    ...
    if not article:
        return None
    article.update(
        id=hashlib.sha256(article["doi"].encode()).hexdigest(),
        journal_id=journal["id"],
        volume=plain(record.get("volume")),
        issue=plain(record.get("issue")),
        pages=plain(record.get("page")),
        article_number=plain(record.get("article-number")),
        sources="crossref",
        first_seen="",
        archive=True,
    )
    ...
    article.update(
        archive_year=int(date[:4]) if date else None,
...
class ArchiveService:
    def __init__(self, directory, registry, getter=get):
        ...
        self.journals = {
            j["id"]: j for j in registry["journals"] if j.get("enabled", True)
        }
        self.getter = getter
        self.lock = threading.Lock()
        with self.connection() as c:
            c.execute(
                "CREATE TABLE IF NOT EXISTS archive_queries (key TEXT PRIMARY KEY, body TEXT)"
            )
            c.execute(
                "CREATE TABLE IF NOT EXISTS archive_articles (id TEXT PRIMARY KEY, journal_id TEXT, year INTEGER, body TEXT)"
            )
            c.execute(
                "CREATE INDEX IF NOT EXISTS archive_year ON archive_articles(journal_id,year)"
            )
    ...
    def cached(self, key):
        with self.connection() as c:
            row = c.execute(
                "SELECT body FROM archive_queries WHERE key=?", (key,)
            ).fetchone()
        return json.loads(row[0]) if row else None

    def save(self, c, key, value):
        c.execute(
            "INSERT OR REPLACE INTO archive_queries VALUES (?,?)",
                ...
                for order in ("asc", "desc"):
                    message = self.getter(
                        ...
                        params={
                            ...
                            "select": "published,published-print,published-online",
                        },
                    ).json()["message"]
                    for item in message["items"]:
                        for field in (
                            ...
                            if (
                                value
                                and 1500 <= int(value[:4]) <= dt.date.today().year + 1
                            ):
            ...
            if advance and not state["complete"]:
                ...
                message = self.getter(
                    ...
                    params={
                        ...
                        "rows": PAGE_SIZE,
                        "cursor": state["cursor"],
                        "select": "DOI,title,ISSN,author,volume,issue,page,article-number,published,published-online,published-print,type",
                    },
                ).json()["message"]
                items = message["items"]
                ...
                state = {
                    **state,
                    "scanned": state["scanned"] + len(items),
                    "phase_scanned": phase_scanned,
                    "checked_at": time.time(),
                    "last_page": [phase, page_dois],
                }
                # Older in-progress caches have no complete membership ledger; don't prune them.
                if "seen" in state:
                    state["seen"] = sorted(
                        set(state["seen"]) | {a["id"] for a in articles}
                    )
                cursor = message.get("next-cursor")
                total = message.get("total-results")
                if len(items) < PAGE_SIZE or (
                    not cursor and type(total) is int and phase_scanned >= total
                ):
                    state.update(
                        phase=phase + 1,
                        cursor="*",
                ...
                with self.connection() as c:
                    for article in corrections:
                        c.execute(
                            "UPDATE archive_articles SET year=?,body=? WHERE id=?",
                            (
                    ...
                    for article in articles:
                        c.execute(
                            "INSERT OR REPLACE INTO archive_articles VALUES (?,?,?,?)",
                            (
                                article["id"],
                                identifier,
                                article["archive_year"],
                    ...
                    if state["complete"] and "seen" in state:
                        seen = set(state["seen"])
                        old = c.execute(
                            "SELECT id FROM archive_articles WHERE journal_id=? AND year=?",
                            (identifier, year),
                        ).fetchall()
                        # Keep saved paper metadata addressable, but remove it from the old year's directory.
                        c.executemany(
                            "UPDATE archive_articles SET year=NULL WHERE id=?",
                            [(row[0],) for row in old if row[0] not in seen],
                        )
    ...
    def result(self, identifier, year, state):
        with self.connection() as c:
            articles = [
                ...
                for row in c.execute(
                    "SELECT body FROM archive_articles WHERE journal_id=? AND year=?",
                    (identifier, year),
                )
            ]
        ...
        with sqlite3.connect(self.history, timeout=30) as c:
            rows = c.execute(
                "SELECT entry_id,doi,abstract FROM matched_entries WHERE journal_id=? AND doi IS NOT NULL",
                (identifier,),
            )
        ...
        return {
            k: v for k, v in state.items() if k not in ("cursor", "last_page", "seen")
        } | {"articles": articles, "reading_aliases": aliases}

    def article(self, identifier):
        with self.connection() as c:
            row = c.execute(
                "SELECT body FROM archive_articles WHERE id=?", (identifier,)
            ).fetchone()
        return json.loads(row[0]) if row else None
