    ...
    try:
        ...
        if addresses and all(ipaddress.ip_address(a) in synthetic for a in addresses):
            response = requests.get('https://dns.google/resolve', params={
                'name': parsed.hostname.encode('idna').decode(), 'type': 'A', 'edns_client_subnet': '0.0.0.0/0'}, timeout=(5, 10))
            response.raise_for_status()
            answer = response.json()
            if answer.get('Status') != 0:raise ValueError()
...
def public_feed_get(url):
    for _ in range(6):
        ...
        cls = urllib3.HTTPSConnectionPool if parsed.scheme == 'https' else urllib3.HTTPConnectionPool
        tls = {'server_hostname': parsed.hostname, 'assert_hostname': parsed.hostname} if parsed.scheme == 'https' else {}
        with cls(address, port=port, **tls) as pool:
            response = pool.urlopen('GET', urlunsplit(('', '', parsed.path or '/', parsed.query, '')),
                headers={'Host': parsed.netloc, 'User-Agent': 'JournalRadar/1.0', 'Accept-Encoding': 'identity'},
                redirect=False, retries=False, preload_content=False, timeout=urllib3.Timeout(connect=10, read=20))
            try:
...
def journal_from_crossref(item):
    ...
    return {'id': identifiers[0], 'name': name, 'short_name': name,
        'issns': identifiers, 'groups': [], 'enabled': True, 'rss_url': None,
        'crossref_identity_verified': True, 'crossref_title': name,
        'publisher': plain(item.get('publisher')),
        'sources': ['https://api.crossref.org/journals/' + identifiers[0]]}


def same_journal(a, b):
    return a['id'] == b['id'] or bool(set(a.get('issns', [])) & set(b.get('issns', []))) or bool(a.get('rss_url') and a['rss_url'] == b.get('rss_url'))


def lookup(query, registry, rss_url='', getter=get, feed_getter=public_feed_get):
    if not isinstance(query, str) or not isinstance(rss_url, str) or len(query) > 2000 or len(rss_url) > 2000:
    ...
    existing = [j for j in registry['journals'] if (identifier and identifier in j['issns']) or j['name'].casefold() == query.casefold() or (rss_url and j.get('rss_url') == rss_url)]
    if existing:
        candidates = copy.deepcopy(existing)
    else:
        try:
            payload = getter('https://api.crossref.org/journals' + ('/' + identifier if identifier else ''),
                **({} if identifier else {'params': {'query': query.replace('&', 'and'), 'rows': 12}})).json()['message']
            candidates = []
    ...
    if feed:
        ...
        if not existing:candidates.append({'id': 'rss-' + hashlib.sha256(rss_url.encode()).hexdigest()[:16],
            'name': plain(feed.feed.title), 'short_name': plain(feed.feed.title), 'issns': [], 'groups': [],
            'enabled': True, 'rss_url': rss_url, 'crossref_enabled': False, 'sources': [rss_url]})
    return candidates
...
def validate_groups(groups, journal_ids):
    ...
    for group in groups:
        ...
        identifier, name, members = group.get('id'), group.get('name'), group.get('journal_ids')
        if not isinstance(identifier, str) or not re.fullmatch(r'[a-z][a-z0-9_-]{0,63}', identifier) or identifier in SYSTEM_GROUPS or identifier in ids:
            raise ValueError('分组标识无效或重复')
        if not isinstance(name, str) or not name.strip() or len(name.strip()) > 40 or name.strip().casefold() in names:
            raise ValueError('分组名称不能为空、重复或超过 40 字')
        if not isinstance(members, list) or any(not isinstance(i, str) or i not in journal_ids for i in members):
            raise ValueError('分组包含尚未添加的期刊')
        ids.add(identifier); names.add(name.strip().casefold())
        result.append({'id': identifier, 'name': name.strip(), 'journal_ids': list(dict.fromkeys(members))})
    return result
...
class Library:
    ...
    def registry(self):
        ...
        registry['groups'] = [g for key, g in groups.items() if key not in self.local['deleted_groups']]
        ...
        for journal in registry['journals']:
            ...
            journal['groups'] += [g['id'] for g in registry['groups'] if journal['id'] in g['journal_ids']]
        return registry
    ...
    def save_groups(self, groups):
        ...
        local['deleted_groups'] = [g['id'] for g in self.base.get('groups', []) if g['id'] not in local['groups']]
        self.save(local)
    ...
    def add(self, candidate, group_id=''):
        ...
        existing = next((j for j in registry['journals'] if same_journal(candidate, j)), None)
        journal = existing or {**copy.deepcopy(candidate), 'groups': [], 'user_added': True}
        local = copy.deepcopy(self.local)
        if not existing:
            local['journals'].append(journal)
        if group_id:
            group = copy.deepcopy(next(g for g in registry['groups'] if g['id'] == group_id))
            group['journal_ids'] = list(dict.fromkeys([*group['journal_ids'], journal['id']]))
