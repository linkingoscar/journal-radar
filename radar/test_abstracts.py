...
def test_bilingual_title_exception_requires_same_doi_and_known_journal():
    ...
    bilingual={**english,'title':A['title']+' Comment le leadership améliore le bien-être des employés'}
    assert from_openalex(work(english),bilingual)==TEXT
    assert not from_openalex(work(english),{**bilingual,'journal_id':'0021-9010'})
    assert not from_openalex(work(english),{**bilingual,'doi':'10.1111/1911-3846.88888'})
    assert not from_openalex({**work(english),'title':'How supportive'},bilingual)


def test_doi_resolution_is_unique_and_journal_verified_and_survives_restart(tmp_path,monkeypatch):
    monkeypatch.setattr('abstracts.time.sleep',lambda _:None)
    article={**A,'doi':'','journal_id':'0021-9010'}
    record={'DOI':A['doi'],'title':[A['title']],'type':'journal-article','ISSN':['0021-9010']}
    records=[record]
    def getter(url):
        if 'api.crossref.org/journals/' in url:return json.dumps({'message':{'items':records}}),url
        if 'api.openalex.org' in url:return json.dumps(work()),url
...
def test_source_backoff_persists_and_other_sources_remain_available(tmp_path,monkeypatch):
    ...
    def getter(url):
        calls.append(url)
        if 'wiley.com' in url:
            response=requests.Response();response.status_code=429;response.url=url;response.headers['Retry-After']='600'
            raise requests.HTTPError(response=response)
        return 'ok',url
    service=AbstractService(tmp_path,getter)
    with pytest.raises(SourceDeferred) as error:service.request('https://onlinelibrary.wiley.com/a')
    assert error.value.reason=='rate_limited'
    restored=AbstractService(tmp_path,getter)
    with pytest.raises(SourceDeferred):restored.request('https://onlinelibrary.wiley.com/b')
    assert len(calls)==1
...
def test_aom_http_redirect_is_upgraded_before_request(monkeypatch):
    class Response:
        encoding='utf-8'
        def __init__(self,status,headers):self.status_code=status;self.headers=headers
        def __enter__(self):return self
        def __exit__(self,*args):pass
        def raise_for_status(self):pass
        def iter_content(self,_):return [b'<html></html>']
    calls=[]
    def get(url,**kwargs):
        calls.append(url)
        return Response(302,{'Location':'http://journals.aom.org/doi/full/10.1000/study'}) if len(calls)==1 else Response(200,{})
    monkeypatch.setattr('abstracts.requests.get',get)
    assert fetch(A['link'])[1]=='https://journals.aom.org/doi/full/10.1000/study'
    assert all(url.startswith('https://') for url in calls)


def test_background_queue_prioritizes_hr35_and_resumes_without_repeating_done_items(tmp_path,monkeypatch):
    monkeypatch.setattr('abstracts.time.sleep',lambda _:None)
    first={**A,'abstract':'','journal_id':'0021-9010','sources':'crossref'}
    second={**first,'id':'b'*64,'doi':'10.1000/second','journal_id':'9999-9999','link':'https://doi.org/10.1000/second'}
    payload={'journals':[{'id':first['journal_id'],'issns':[first['journal_id']],'groups':['hr35']},{'id':second['journal_id'],'issns':[second['journal_id']],'groups':[]}],'articles':[second,first]}
    calls=[];stop=threading.Event()
    def getter(url):
        calls.append(url)
        if 'api.openalex.org' in url:return '{"results":[]}',url
        identifier='10.1000/second' if 'second' in url else A['doi']
        return '<meta name="citation_doi" content="'+identifier+'"><div class="abstract">'+TEXT+'</div>',url
    service=AbstractService(tmp_path,getter)
    def progress(report):
        if report['stage']=='pages' and report['checked']==1:stop.set()
