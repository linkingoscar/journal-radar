...
def record(doi="10.1000/study", **fields):
    return {
        "DOI": doi,
        "title": ["How leaders support wellbeing at work"],
        "ISSN": J["issns"],
        "type": "journal-article",
        "volume": "65",
        "issue": "1",
        "page": "21-35",
        "published-print": {"date-parts": [[1980, 1]]},
        "published-online": {"date-parts": [[1979, 12]]},
        **fields,
    }
...
def test_archive_year_uses_print_year_and_preserves_volume_only_and_combined_issues():
    ...
    a = normalize_archive(
        record(
            volume="",
            issue="",
            **{"published-print": {}, "published-online": {"date-parts": [[2026, 9]]}},
        ),
        J,
    )
    assert a["archive_year"] == 2026 and a["year_basis"] == "publication"
    with pytest.raises(ValueError):
        normalize_archive(record(ISSN=["0000-0000"]), J)


def test_opening_service_does_not_fetch_and_year_pages_resume_after_failure(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("archives.PAGE_SIZE", 2)
    calls = []
    replies = [
        response([record("10.1000/a"), record("10.1000/b")]),
    ...
    with pytest.raises(RuntimeError):
        archive.year(J["id"], 1980, advance=True)
    ...
    assert (
        "from-print-pub-date" in calls[0]["filter"]
        and "from-pub-date" in calls[-1]["filter"]
    )
    ...
    replies = [
        ...
        response(
            [
                item,
                record(
                    "10.1000/other", **{"published-print": {"date-parts": [[1981, 1]]}}
                ),
            ]
        ),
    ]
    archive = ArchiveService(
        tmp_path, {"journals": [J]}, lambda url, params: replies.pop(0)
    )
    first = archive.year(J["id"], 1980)
...
def test_overview_is_cached_and_refresh_failure_preserves_completed_year(tmp_path):
    ...
    with pytest.raises(ValueError):
        archive.year("0000-0000", 1980)
    with pytest.raises(ValueError):
        archive.year(J["id"], 1499)
...
def test_refresh_removes_old_directory_members_only_after_success_and_retains_paper(
    tmp_path,
):
    replies = [
        response([record()]),
    ...
    archive = service(tmp_path, getter)
    archive.year(J["id"], 1980)
    before = archive.year(J["id"], 1980, advance=True)
...
def test_full_final_page_without_cursor_is_complete_when_total_confirms_end(
    tmp_path, monkeypatch
):
    monkeypatch.setattr("archives.PAGE_SIZE", 1)
    values = [
        SimpleNamespace(
            json=lambda: {"message": {"items": [record()], "total-results": 1}}
        ),
        response([]),
    ]
...
def test_apa_alias_merge_requires_matching_identity_and_preserves_reading_aliases(
    tmp_path,
):
    a = record(
        "10.1037//0021-9010.65.1.1", author=[{"given": "A", "family": "Researcher"}]
    )
    b = {
        **a,
        "DOI": "10.1037/0021-9010.65.1.1",
        "published-print": {},
        "published-online": {"date-parts": [[1980]]},
    }
    store = RadarStore(tmp_path)
    from run import normalize_crossref
    ...
    archive = ArchiveService(
        tmp_path, {"journals": [J]}, lambda url, params: response([a, b])
    )
