  ...
  class Node {
    ...
    querySelectorAll() {
      return this.children
        ...
        .filter((c) => c.dataset.archiveFocus);
    }
    focus() {
...
test('unrelated refresh preserves the existing directory DOM and keyboard focus', () => {
  const { view, nodes, document } = fixture();
  view.paintArticles();
  const list = nodes['#archive-articles'];
  const focused = list.querySelectorAll().find((n) => n.dataset.archiveFocus === 'saved:a');
  focused.focus();
...
test('changing a reading flag restores focus to the same action after repaint', () => {
  const { view, nodes, state, document } = fixture();
  view.paintArticles();
  nodes['#archive-articles']
    .querySelectorAll()
    .find((n) => n.dataset.archiveFocus === 'saved:a')
    .focus();
  state.saved.a = true;
  view.paintArticles();
...
test('returning to a cached year while another year is pending restores its rows', async () => {
  ...
  for (const id of ['archive-year', 'archive-years', 'archive-issues'])
    nodes['#' + id] = { replaceChildren() {} };
  let finishPending;
  view.request = async (_id, query) =>
    query.includes('year=1981')
      ? new Promise((resolve) => {
  ...
  finishPending(cached);
  await pending;
  assert.equal(nodes['#archive-articles'].children.length, 1);
  assert.equal(
    nodes['#archive-articles'].querySelectorAll().find((n) => n.dataset.archiveFocus === 'open:a')
      .textContent,
    'An existing article',
  );
});
...
test('switching issue preserves the focused issue control', () => {
  ...
  view.issue = view.key(view.result.articles[0]);
  view.paintIssues();
...
test('a stale current year is refreshed once on entry while old rows survive a failed refresh', async () => {
  const { view, nodes } = fixture(),
    ...
    cached = { ...view.result, year, checked_at: Date.now() / 1000 - 90000 };
  Object.assign(view, {
    result: cached,
    desktop: true,
    generation: 0,
  ...
  view.request = async (_id, query) => {
    queries.push(query);
    if (query.includes('refresh')) throw new Error('offline');
    return cached;
  };
  await view.loadYear(year);
  assert.equal(queries.length, 2);
  assert(queries[1].endsWith('&refresh=1'));
  assert.equal(nodes['#archive-articles'].children.length, 1);
});
