...
def test_cloud_import_keeps_reading_identity_and_merges_local_rss(tmp_path):
    ...
    row = {
        ...
        "link": "https://doi.org/10.1000/paper",
        "doi": "10.1000/paper",
    ...
    assert (
        records[0]["abstract"] == "An abstract"
        and records[0]["sources"] == "crossref,rss"
    )
...
def test_loopback_api_rejects_cross_origin_rebinding_and_unauthenticated_mutations(
    ...
    app.reading_articles = lambda ids: {
        "articles": [{"id": identifier} for identifier in ids]
    }
    app.abstract_slot = threading.BoundedSemaphore(1)
    app.abstracts = SimpleNamespace(
        lookup=lambda article: calls.append(article["id"])
        or {"status": "found", "abstract": "An existing abstract"}
    )
    app.abstracts.overlay = lambda result: result
    app.archive_slot = threading.BoundedSemaphore(1)
    app.archives = SimpleNamespace(
        overview=lambda jid: {"journal_id": jid},
        year=lambda jid, year, **kwargs: {"articles": [], "year": year},
    )
    ...
    try:
        assert (
            requests.get(url + "/api/session").json()["app"] == "journal-radar-desktop"
        )
        reading = url + "/api/reading-articles?ids=" + "a" * 64
        assert requests.get(reading).json()["articles"] == [{"id": "a" * 64}]
        assert (
            requests.get(
                reading, headers={"Origin": "https://untrusted.test"}
            ).status_code
            == 403
        )
        ...
        assert (
            requests.get(
                url + "/api/reading-articles?ids=" + ",".join(["a" * 64] * 101)
            ).status_code
            == 400
        )
        for cover, mime in [
            ("cover-0001-4273.jpg", "image/jpeg"),
            ("cover-0001-8392.webp", "image/webp"),
            ("cover-1572-3097.jpeg", "image/jpeg"),
        ]:
        ...
        assert (
            requests.get(
                url + "/api/session", headers={"Origin": "https://untrusted.test"}
            ).status_code
            == 403
        )
        assert (
            requests.get(
                url + "/api/session", headers={"Host": f"untrusted.test:{port}"}
            ).status_code
            == 403
        )
        ...
        assert (
            requests.post(
                url + "/api/sync", headers={"Origin": url, "X-Radar-Token": app.token}
            ).status_code
            == 202
        )
        assert calls == [True]
        ...
        assert (
            requests.post(
                ...
                data="https://untrusted.test",
            ).status_code
            == 400
        )
        assert (
            requests.post(
                url + "/api/abstract/" + "c" * 64, headers={"X-Radar-Token": app.token}
            ).status_code
            == 404
        )
        assert (
            requests.post(endpoint, headers={"X-Radar-Token": app.token}).json()[
                "status"
            ]
            == "found"
        )
        ...
        assert requests.post(
            url + "/api/abstracts/pause", headers={"X-Radar-Token": app.token}
        ).json()["paused"]
        ...
        assert (
            requests.post(
                archive + "?year=1980", headers={"X-Radar-Token": app.token}
            ).json()["year"]
            == 1980
        )
        assert (
            requests.post(
                archive + "?year=wrong", headers={"X-Radar-Token": app.token}
            ).status_code
            == 400
        )
        assert (
            requests.post(
                archive + "?year=1980&year=1981", headers={"X-Radar-Token": app.token}
            ).status_code
            == 400
        )
        assert (
            requests.post(
                archive + "?url=https://untrusted.test",
                headers={"X-Radar-Token": app.token},
            ).status_code
            == 400
        )
        ...
        assert (
            requests.post(archive, headers={"X-Radar-Token": app.token}).status_code
            == 429
        )
...
def test_sync_runs_abstract_queue_and_publishes_results(tmp_path, monkeypatch):
    ...
    app.sync()
    assert (
        not app.status["running"]
        and app.status["paused"]
        and app.status["error"] is None
    )
    assert json.loads(
        (tmp_path / "abstract-progress.json").read_text(encoding="utf-8")
    )["paused"]
...
def test_cancelled_browser_request_does_not_retry_writing_to_closed_connection():
    ...
    def closed():
        raise ConnectionAbortedError("Browser switched years")

    handler.end_headers = closed
    assert handler.respond(200, {"complete": False}) is None


def test_desktop_retries_failed_crossref_and_exposes_local_health(
        ...
        "journals": [
            {
                **J,
                "health": [
                    {"source": "crossref", "error": "429", "last_success": now()}
                ],
            }
        ],
        "articles": [],
    ...
    calls = []
    monkeypatch.setattr(
        "desktop.collect_crossref",
        lambda journal, last, attempt, days: calls.append((journal["id"], last)) or [],
    )
    monkeypatch.setattr(
        app.abstracts,
        "enrich",
        lambda *args: {"found": 0, "remaining": 0, "paused": False},
    )
    app.sync()
    ...
    assert any(
        h["source"] == "本机 Crossref" and not h["error"]
        for h in payload["journals"][0]["health"]
    )
...
def test_reading_metadata_recovers_cached_history_without_external_requests(tmp_path):
    ...
    (tmp_path / "site/data.json").write_text(
        json.dumps({"articles": [recent]}), encoding="utf-8"
    )
    with app.archives.connection() as c:
        c.execute(
            "INSERT INTO archive_articles VALUES (?,?,?,?)",
            (archived["id"], J["id"], 1980, json.dumps(archived)),
        )
    app.archives.getter = lambda *a, **k: pytest.fail(
        "Reading metadata must not trigger a remote query"
    )
    result = app.reading_articles(
        [recent["id"], archived["id"], "f" * 64, archived["id"]]
    )
    ...
    legacy = normalize_archive(
        record(doi="10.1037//0021-9010.65.1.21", author=author), J
    )
    canonical = normalize_archive(
        record(doi="10.1037/0021-9010.65.1.21", author=author), J
    )
    with app.archives.connection() as c:
        for a in (legacy, canonical):
            c.execute(
                "INSERT INTO archive_articles VALUES (?,?,?,?)",
                (a["id"], J["id"], 1980, json.dumps(a)),
            )
    migrated = app.reading_articles([legacy["id"], canonical["id"]])
    assert (
        len(migrated["articles"]) == 1
