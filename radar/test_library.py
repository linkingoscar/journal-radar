...
J = {
    ...
    "issns": ["0093-5301", "1537-5277"],
    "groups": ["ft50", "marketing"],
}
BASE = {
    "journals": [J],
    "groups": [{"id": "marketing", "name": "Marketing", "journal_ids": [J["id"]]}],
}


def test_groups_membership_rename_delete_restart_and_new_seed(tmp_path):
    path = tmp_path / "library.json"
    lib = Library(BASE, path)
    groups = [
        {"id": "marketing", "name": "消费者研究", "journal_ids": []},
        {"id": "g-second", "name": "第二组", "journal_ids": [J["id"]]},
    ]
    ...
    updated = {
        **copy.deepcopy(BASE),
        "groups": [
            *BASE["groups"],
            {"id": "new-seed", "name": "New seed", "journal_ids": [J["id"]]},
        ],
    }
...
def test_issn_alias_dedup_and_atomic_validation(tmp_path, monkeypatch):
    ...
    assert (
        existing and journal["id"] == J["id"] and len(lib.registry()["journals"]) == 1
    )
    before = copy.deepcopy(lib.local)
    with pytest.raises(ValueError):
        lib.save_groups(
            [{"id": "g-invalid", "name": "Broken", "journal_ids": ["missing"]}]
        )
    assert lib.local == before
    with pytest.raises(ValueError):
        lib.add(alias, "missing")
    monkeypatch.setattr(
        type(lib.path),
        "replace",
...
def test_lookup_existing_and_rss_without_crossref():
    def no_network(*a, **k):
        pytest.fail("Existing ISSN must not need Crossref")

    assert lookup("15375277", BASE, getter=no_network)[0]["id"] == J["id"]
    with pytest.raises(ValueError):
        lookup("0093-5302", BASE, getter=no_network)
    ...
    candidate = lookup(
        ...
        BASE,
        getter=unavailable,
        feed_getter=lambda _: SimpleNamespace(content=feed),
    )[0]
    assert (
        candidate["id"].startswith("rss-")
        and candidate["crossref_enabled"] is False
        and candidate["issns"] == []
    )
    assert (
        normalize_rss(
            {"title": "A paper", "link": "https://example.org/paper"}, candidate
        )["title"]
        == "A paper"
    )
...
def test_public_rss_pins_validated_ip_and_rejects_private_redirect(monkeypatch):
    ...
    with pytest.raises(ValueError, match="公网"):
        public_feed_get("https://example.org/feed")
    assert len(connections) == 1 and connections[0][0] == "93.184.216.34"
    assert (
        connections[0][1]["server_hostname"]
        == connections[0][1]["assert_hostname"]
        == "example.org"
    )
    with pytest.raises(ValueError):
        public_feed_get("file:///etc/passwd")


def test_proxy_synthetic_dns_uses_verified_public_answer_and_still_blocks_private(
    monkeypatch,
):
    monkeypatch.setattr(
        socket, "getaddrinfo", lambda *a, **k: [(2, 1, 6, "", ("198.18.0.12", 443))]
    )
    ...
    assert (
        calls[0][0] == "https://dns.google/resolve"
        and calls[0][1]["params"]["name"] == "example.org"
    )
    answer["Answer"][0]["data"] = "127.0.0.1"
    with pytest.raises(ValueError):
        public_target("https://example.org/feed")


def test_authenticated_library_api_preserves_reading_and_rejects_stale_writes(
    ...
    app.store.ingest(
        ...
        [
            normalize_rss(
                {"title": "Saved research", "link": "https://example.org/paper"}, J
            )
        ],
    )
    ...
    headers = {"X-Radar-Token": app.token}
    try:
        ...
        assert (
            requests.post(url + "groups", headers=headers, data="{}").status_code == 400
        )
        assert (
            requests.post(url + "groups", headers=headers, json=[]).status_code == 400
        )
        revision = app.library_revision()
        groups = [
            *app.registry["groups"],
            {"id": "g-test", "name": "多组测试", "journal_ids": []},
        ]
        assert (
            requests.post(
                url + "groups",
                headers=headers,
                json={"groups": groups, "revision": revision},
            ).status_code
            == 200
        )
        assert (
            requests.post(
                url + "groups",
                headers=headers,
                json={"groups": [], "revision": revision},
            ).status_code
            == 400
        )
        candidate = requests.post(
            url + "lookup", headers=headers, json={"query": "1537-5277"}
        ).json()["journals"][0]
        ...
        assert next(g for g in app.registry["groups"] if g["id"] == "g-test")[
            "journal_ids"
        ] == [J["id"]]
        ...
        bad = requests.post(
            url + "journals",
            headers=headers,
            json={"candidate": "forged", "revision": app.library_revision()},
        )
        ...
        with app.store.get_connection("history") as connection:
            assert (
                connection.execute("SELECT title FROM matched_entries").fetchone()[
                    "title"
                ]
                == "Saved research"
            )
...
def test_new_journal_reaches_archives_and_is_collected_without_cloud_entry(
    tmp_path, monkeypatch
):
    app = Companion(tmp_path)
    journal = {
    ...
    started = []
    monkeypatch.setattr(app, "start_sync_unlocked", lambda: started.append(True))
    app.change_library(
    ...
    monkeypatch.setattr(
        "desktop.get",
        lambda _: SimpleNamespace(json=lambda: {"journals": [], "articles": []}),
    )
    calls = []
    monkeypatch.setattr(
        "desktop.collect_crossref", lambda j, *a: calls.append(j["id"]) or []
    )
    monkeypatch.setattr(
        app.abstracts,
        "enrich",
        lambda *a: {"found": 0, "remaining": 0, "paused": False},
    )
    app.sync()
    ...
    assert any(
        j["id"] == journal["id"] for j in Companion(tmp_path).registry["journals"]
    )
...
def test_portable_backup_restores_new_journals_groups_and_rejects_unsafe_config(
    tmp_path,
):
    app = Companion(tmp_path)
    nature = {
    ...
    incoming = {
        ...
        "groups": [
            {
                "id": "my-import",
                "name": "My imported papers",
                "journal_ids": [nature["id"]],
            }
        ],
        "deleted": [],
    }
    revision = app.library_revision()
    app.change_library("restore", {"revision": revision, "library": incoming})
    assert app.archives.journals[nature["id"]]["user_added"]
    restored = Companion(tmp_path)
    assert next(g for g in restored.registry["groups"] if g["id"] == "my-import")[
        "journal_ids"
    ] == [nature["id"]]
    before = restored.library_revision()
    with pytest.raises(ValueError):
        restored.change_library(
            "restore",
            {
                "revision": before,
                "library": {
                    **incoming,
                    "journals": [{**nature, "rss_url": "http://127.0.0.1/private"}],
                },
            },
        )
    assert restored.library_revision() == before
