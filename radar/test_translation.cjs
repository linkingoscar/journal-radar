...
const memory = () => {
  ...
  return {
    async get(k) {
      return structuredClone(entries.get(k));
    },
    async put(k, v) {
...
test('restored translations work offline without resetting usage or overwriting an existing translation', async () => {
  const store = memory(),
    usage = [{ at: Date.now(), chars: 5000 }];
  await store.put('usage', usage);
  const engine = new Engine({
    store,
    fetcher: async () => {
      throw new Error('must not use network');
    },
  });
  ...
  assert.equal(cached.text, '从备份恢复的译文');
...
test('UTF-8 chunks preserve every source character within the provider byte limit', () => {
  const source = 'Employee well-being matters. 员工😀参与 and organizational performance! '.repeat(
    35,
  );
  ...
  const sentences =
    ...
    'A second sentence contains detailed findings about organizational behavior. '.repeat(9);
...
test('concurrent requests translate once; a new engine reuses the persisted full cache', async () => {
  ...
  const results = await Promise.all([
    engine.translate('An academic abstract.'),
    engine.translate('An academic abstract.'),
  ]);
  assert.equal(calls, 1);
  assert.equal(results[0].text, '中文译文');
...
test('quota blocks new requests but never blocks already cached translations', async () => {
  ...
  assert.equal((await engine.translate('Previously translated abstract.')).cached, true);
  assert.equal(calls, 1);
});
...
test('a failed segment is not cached as a translation; retry resumes completed segments', async () => {
  ...
  const engine = new Engine({
    ...
    fetcher: async () => {
      calls++;
      return calls === 2
        ? { ok: true, json: async () => ({ responseStatus: 429, quotaFinished: true }) }
        : response('员工动机与绩效。');
    },
  });
  await assert.rejects(engine.translate(source), /额度已用完/);
  const result = await engine.translate(source);
  assert(result.text.includes('员工动机与绩效'));
  assert.equal(calls, splitText(source.trim()).length + 1);
});
...
test('an already cancelled reader consumes no provider quota', async () => {
  ...
  await assert.rejects(engine.translate('An abstract.', { signal: controller.signal }), {
    name: 'AbortError',
  });
  assert.equal(calls, 0);
  assert.equal(await store.get('usage'), undefined);
});
...
test('email mode sends de safely, preserves cache and usage across upgrades and removal', async () => {
  ...
  assert.equal(urls[1].searchParams.get('de'), 'scholar+radar@example.org');
  assert.equal(
    (await store.get('usage')).reduce((n, x) => n + x.chars, 0),
    LIMIT + 'New abstract.'.length,
  );
  email = '';
  await assert.rejects(engine.translate('Another abstract.'), /匿名版约 5,000/);
  assert.equal((await engine.translate('New abstract.')).cached, true);
  email = 'scholar@example.org';
  await store.put('usage', [{ at: clock(), chars: EMAIL_LIMIT }]);
