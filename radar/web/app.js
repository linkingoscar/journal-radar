...
function hydrateReading(){
  ...
  try{
    ...
    if(isDesktop)for(let i=0;i<ids.length;i+=100){
      const r=await fetch('api/reading-articles?ids='+ids.slice(i,i+100).join(','),{cache:'no-store'});
      if(!r.ok)throw new Error('旧阅读记录的文章信息暂未恢复，请稍后刷新。');
      const result=await r.json();Object.assign(archiveReadingAliases,result.reading_aliases||{});applyReadingAliases();for(const a of result.articles)rememberArticle(a);
    }
...
const libraryUI=new JournalLibrary.Manager({desktop:isDesktop,data:()=>data,group:()=>group,reload:()=>load(),toast});
libraryUI.bind();
...
function label(g) { return libraryUI.label(g); }
...
function applyRoute(focus=false){
  ...
  if(location.hash==='#saved'){
    browseMode='saved';group='all';view='saved';$('#search').value='';$('#period').value='all';$('#abstract-filter').value='all';
  }else if(journal){
    browseMode='journal';if(!inGroup(journal))group='all';
    $('#search').value='';$('#period').value='all';$('#abstract-filter').value='all';view='all';
  }else{
...
function renderCatalog(journals){
  ...
  for(const j of visible){
    ...
    const body=el('div',undefined,'catalog-body');body.append(el('h2',j.name),el('p',meta.discipline||j.publisher||'学术期刊','catalog-discipline'),el('p',j.issns.length?'ISSN '+j.issns.join(' / '):'RSS 订阅','catalog-issn'));
...
function renderAbstract(article,journal,content){
  ...
  const manual=el('details'),manualTitle=el('summary','粘贴原文摘要'),manualText=el('textarea');manualText.rows=7;manualText.maxLength=20000;manualText.setAttribute('aria-label','原文摘要');
  const saveManual=button('保存原文摘要',()=>{try{const saved=JournalReading.manual(article,manualText.value);abstractController?.abort();Object.assign(article,saved);rememberArticle(article);renderAbstract(article,journal,content);render();toast('已保存原文摘录，导出阅读记录时会一并备份。');}catch(error){toast(error.message);}});
  manual.append(manualTitle,el('p','从出版商原文复制摘要。保存后会明确标为手动摘录，不会伪装成自动核验结果。','translation-note'),manualText,saveManual);content.append(manual);
  if(!isDesktop){
  ...
  const retrieve=async()=>{
    ...
    try{
      ...
      if(result.abstract){
        ...
        rememberArticle(article);renderAbstract(article,journal,content);render();
...
function openArticle(article) {
  ...
  if(article.doi||article.resolved_doi)content.append(el('p','DOI '+(article.doi||article.resolved_doi),'reader-doi'));
...
function render() {
  ...
  archives.sync();
  $('#journal-library').hidden=!isLibrary;$('#article-feed').hidden=isLibrary||!!current&&archives.mode==='archive';
  $('#back-library').hidden=!current;$('#back-library').href='#library='+group;
  $('#library-mode').href='#library='+group;$('#feed-mode').href='#feed='+group;
  for(const [id,active] of [['library-mode',['library','journal'].includes(browseMode)],['feed-mode',browseMode==='feed'],['saved-mode',browseMode==='saved']]){if(active)$('#'+id).setAttribute('aria-current','page');else $('#'+id).removeAttribute('aria-current');}
  ...
  const all=allKnown().filter(a=>ids.has(a.journal_id)&&(view==='saved'||browseMode==='saved'||recentIds.has(a.id))&&(browseMode!=='saved'||state.saved[a.id]));
  ...
  $('#group-description').textContent=group==='hr35'?'你指定的 35 本期刊，涵盖人力资源、组织行为与管理研究。':group==='ft50'?'FT50 · 2026 年 4 月版，50 本期刊的研究动态。':group==='utd24'?'UTD24 · 跨管理、金融、营销、会计与信息系统。':group==='custom'?'在「管理期刊与数据源」中选择你想单独关注的期刊。':group==='all'?'全部期刊汇聚于此，重叠清单合并展示。':`${label(group)} · ${journals.length} 本期刊，可在「管理分组」中调整。`;
  ...
  if(current){title.textContent=current.name;$('#group-description').textContent=(current.issns.length?'ISSN '+current.issns.join(' / '):'RSS 订阅')+' · '+current.groups.map(label).join(' · ');$('#article-total').textContent=all.filter(a=>a.journal_id===current.id).length.toLocaleString();}
  if(current)$('#total-label').textContent='篇近期已收录';
  if(browseMode==='saved'){title.textContent='我的全部收藏';$('#group-description').textContent='跨期刊、跨年份汇总近期与历史收藏；文章信息随阅读记录一起备份。';$('#article-total').textContent=all.filter(a=>state.saved[a.id]).length;$('#total-label').textContent='篇收藏文章';}
  ...
  $('#coverage-status').textContent=browseMode==='saved'&&unknown?`${unknown} 条旧收藏暂缺文章信息。本机可从已查询目录恢复；其他设备请导入新版阅读备份。`:`当前范围 ${all.length} 篇，${missing} 篇缺摘要。可筛选缺摘要文章，按篇补取或粘贴原文摘要。来源请求成功不代表摘要齐全。`;
  ...
  for(const article of articles.slice(0,limit)){
    ...
    if(article.article_type&&article.article_type!=='journal-article'&&article.article_type!=='rss-entry')tags.append(el('span',article.article_type,'tag'));
    if(article.archive)tags.append(el('span',(article.archive_year||'往期')+' 年历史文章','tag'));
...
const archives=new JournalArchives({root:$('#journal-archive'),desktop:isDesktop,reading:()=>state,open:openArticle,toggle,onMode:render,
  ...
  request:async(identifier,query,signal)=>{
    ...
    for(let attempt=0;attempt<60;attempt++){
      ...
      const response=await fetch('api/archive/'+identifier+query,{method:'POST',headers:{'X-Radar-Token':desktopSession.token},signal:AbortSignal.any([signal,AbortSignal.timeout(90000)])});
      if(response.status===429){await new Promise(resolve=>setTimeout(resolve,1000));continue;}
      const result=await response.json();if(!response.ok)throw new Error(result.error||'目录加载失败');return result;
    }
...
async function load(){
  ...
  try{
    ...
    for(const journal of next.journals)journal.groups=journal.groups.filter(g=>g!=='core10');
    const firstLoad=!data,previous=$('#journal').value;libraryUI.apply(next);data=next;
    if(!libraryUI.has(group)){group='all';history.replaceState(null,'','#library=all');browseMode='library';journalId=null;archives.enter(null);}
    applyReadingAliases();updateJournals();if([...$('#journal').options].some(o=>o.value===previous))$('#journal').value=previous;if(firstLoad)applyRoute();else render();
...
function settings(){
  ...
  for(const j of data.journals){
    ...
    const articles=data.articles.filter(a=>a.journal_id===j.id),missing=articles.filter(a=>!abstractText(a)).length;
    row.append(el('small',`近期收录 ${articles.length} 篇 · ${missing} 篇缺摘要 · ${articles.length?Math.round((articles.length-missing)/articles.length*100):0}% 有摘要`));
...
['search','journal','period','sort','abstract-filter'].forEach(id=>$('#'+id).addEventListener(id==='search'?'input':'change',()=>{limit=40;render();}));
...
$('#import-file').addEventListener('change',async event=>{
  ...
  try{
    ...
    const input=JSON.parse(await file.text()),restored=validateState(input),backup=JournalReading.backup(input);
...
if('serviceWorker' in navigator){
  ...
  navigator.serviceWorker.addEventListener('controllerchange',()=>{if(loadedWithWorker)$('#update-banner').hidden=false;});
...
$('#apply-update').addEventListener('click',()=>{const url=new URL(location.href);url.searchParams.set('version',Date.now());location.replace(url.href);});
async function pollDesktop(){
  try{const response=await fetch('api/session',{cache:'no-store'});if(!response.ok)throw new Error();const next=await response.json();if(next.app!=='journal-radar-desktop')throw new Error();desktopSession=next;$('#sync-local').disabled=next.running;$('#sync-local').textContent=next.paused?'继续补采':'本机补采';$('#pause-abstracts').hidden=!next.running||!next.abstract_stage;$('#desktop-status').textContent=next.phase+(next.running&&next.total?`（${next.completed}/${next.total}）`:'')+(next.error?' · '+next.error:'');if(next.data_revision!==desktopRevision){desktopRevision=next.data_revision;await load();}}
