...
const JournalReading=(()=>{
  ...
  function citation(value){
    if(!value||typeof value!=='object'||Array.isArray(value))throw new Error('引用信息无效');
    const result={};
    for(const key of ['title','journal','year','volume','issue','pages','article_number','doi','status','checked_at']){
      const v=value[key]??'';if(typeof v!=='string'||v.length>4000)throw new Error('引用字段无效');result[key]=v;
    }
    if(result.year&&!/^[12]\d{3}$/.test(result.year))throw new Error('引用年份无效');
    if(!['','advance online publication'].includes(result.status))throw new Error('引用发表状态无效');
    if(value.manual===true)result.manual=true;
    if(value.authors_incomplete===true)result.authors_incomplete=true;
    if(!Array.isArray(value.authors)||value.authors.length>2000)throw new Error('引用作者无效');
    result.authors=value.authors.map(a=>{
      if(!a||typeof a!=='object')throw new Error('引用作者无效');const name={};
      for(const k of ['family','given','literal'])if(a[k]!==undefined){if(typeof a[k]!=='string'||a[k].length>500)throw new Error('引用作者无效');name[k]=a[k];}
      if(!name.family?.trim()&&!name.literal?.trim())throw new Error('引用作者缺少姓或机构名');return name;
    });
    return result;
  }
  function folders(value=[],saved){
    ...
    const seen=new Set();
    return value.map(f=>{
      if(!f||!/^folder-[a-z0-9-]{1,80}$/.test(f.id)||seen.has(f.id)||typeof f.name!=='string'||!f.name.trim()||f.name.length>100||!Array.isArray(f.articles)||f.articles.length>20000||f.articles.some(id=>typeof id!=='string'||!(/^[a-f0-9]{64}$/).test(id)))throw new Error('收藏分组格式无效');
      seen.add(f.id);return {id:f.id,name:f.name.trim(),articles:[...new Set(f.articles)].filter(id=>!saved||saved[id])};
    });
  }
  function mergeFolders(current,incoming,saved){
    const result=new Map(folders(current).map(f=>[f.id,f]));
    for(const f of folders(incoming)){const old=result.get(f.id);result.set(f.id,old?{...old,articles:[...new Set([...old.articles,...f.articles])]}:f);}
    return folders([...result.values()],saved);
  }
  function remapFolders(value,aliases,saved){return folders(value).map(f=>({...f,articles:[...new Set(f.articles.map(id=>aliases[id]||id))].filter(id=>saved[id])}));}
  function merge(recent,remembered,aliases={}){
    ...
    for(const a of [...remembered,...recent]){
      ...
      if(old?.citation&&(!a.citation||(old.citation.manual&&!a.citation.manual)||(old.citation.checked_at||'')>(a.citation.checked_at||'')))next.citation=old.citation;
      rows.set(id,next);
    }
    return [...rows.values()];
  }
  ...
  function backup(input){
    if(!input||![1,2,3].includes(input.version)||!input.read||!input.saved||!Array.isArray(input.custom))throw new Error('备份格式不匹配');
